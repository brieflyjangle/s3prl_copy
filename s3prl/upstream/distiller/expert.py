...
class UpstreamExpert(UpstreamBase):
    ...
    def __init__(self, ckpt, model_config=None, **kwargs):
        ...
        # Override with custom model config if provided
        if model_config is not None:
            print(
                "[UpstreamExpert] - Using upstream expert config file from:",
                model_config,
            )
            # model_config is the path to config_model.yaml, not options
            # Don't override options, just pass the config path
        else:
            print("[UpstreamExpert] - Using the default upstream expert config")

        options["ckpt_file"] = ckpt

        self.model = PretrainedDistiller(options, config=model_config)
